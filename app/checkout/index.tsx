...
type PaymentMethod = "CARD" | "PAYPAL" | "APPLE" | "CASH";
...
export default function Screen() {
  ...
  const paymentOptions: Array<{
    ...
    { id: "CASH", label: "Pagar en caja", icon: "cash-outline" },
  ];
  ...
  const handlePayNow = async () => {
    ...
    // No gateway credentials required here yet: this only confirms and creates a local order.
    let rpcResult = await supabase.rpc("create_local_order");

    if (rpcResult.error) {
      const msg = rpcResult.error.message.toLowerCase();
      const isSignatureIssue =
        msg.includes("does not exist") || msg.includes("function") || msg.includes("parameter");
      if (isSignatureIssue) {
        rpcResult = await supabase.rpc("create_local_order", { p_table_number: null });
      }
    }
    ...
    if (rpcResult.error) {
      const msg = rpcResult.error.message.toLowerCase();
      const needsTable =
        msg.includes("mesa") || msg.includes("table") || msg.includes("numero de mesa");
      if (needsTable) {
        // Temporary fallback while table picker is not in UI.
        rpcResult = await supabase.rpc("create_local_order", { p_table_number: 1 });
      }
    }
  ...
  return (
    <ScreenView style={[styles.container, { backgroundColor: palette.bg }]}> 
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: palette.text }]}>Checkout</Text>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        {loading ? (
          <View style={styles.loadingRow}>
            ...
            <Text style={[styles.loadingText, { color: palette.textMuted }]}>Cargando...</Text>
          </View>
        ) : null}
        ...
        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <Text style={[styles.cardTitle, { color: palette.text }]}>Metodo de pago</Text>
              ...
              <Pressable
                ...
                style={[
                  styles.payOption,
                  {
                    ...
                    backgroundColor: active ? palette.accentSoft : palette.bg,
                  },
                ]}
              >
                <View style={styles.payLeft}>
                  <View style={[styles.payIconWrap, { borderColor: active ? palette.accent : palette.border }]}>
                    <Ionicons
                      name={option.icon}
                      size={15}
                  ...
                  <Text style={[styles.payLabel, { color: active ? palette.accent : palette.text }]}>
                    {option.label}
                  </Text>
                </View>
                <Ionicons
        ...
        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <Text style={[styles.cardTitle, { color: palette.text }]}>Detalle de orden</Text>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text style={[styles.summaryValue, { color: palette.text }]}>Q{fee.toFixed(2)}</Text>
          </View>
          <View style={[styles.divider, { backgroundColor: palette.border }]} />
          <View style={styles.summaryRow}>
            <Text style={[styles.totalLabel, { color: palette.text }]}>Total</Text>
            <Text style={[styles.totalValue, { color: palette.text }]}>Q{total.toFixed(2)}</Text>
          </View>
        </View>
        ...
        <Pressable
          ...
          style={[
            styles.payButton,
            { backgroundColor: paying || rows.length === 0 ? palette.border : palette.accent },
          ]}
        >
          <Text style={styles.payButtonText}>{paying ? "Procesando..." : "Pagar ahora"}</Text>
        </Pressable>
      </ScrollView>
    </ScreenView>
  );
